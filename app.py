...
def fetch_movie(title):
    ...
    try:
        ...
        if data.get("results"):
            ...
            poster_url = (
                f"https://image.tmdb.org/tp/w500{poster_path}"
...
if "genre_id_map" not in st.session_state:
    try:
        ...
        g_res = requests.get(g_url, params={"api_key": TMDB_API_KEY})
        genres = g_res.json().get("genres", [])
        st.session_state["genre_id_map"] = {g["id"]: g["name"] for g in genres}
    except:
...
with prediction_col:
    ...
    if st.button("Run Analytics Engine Pipeline", type="primary"):
        ...
        # Combine talent scores dynamically
        ...
        st.info(f"**Engine Framework Allocation:** {pred['risk_level']}")
        
        t1, t2 = st.tabs(["Pillar Weight Details", "Strategic Guidance Log"])
...
# =====================================================
# RIGHT PANEL: SEARCH & HISTORY LOOKUPS
# =====================================================
...
with search_col:
    ...
    if movie_data:
        ...
        for movie in similar_movies:
            st.markdown(
                f"• {movie.get('title')} ({movie.get('release_date', '####')[:4]})"
            )
