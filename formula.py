def calculate_cinema_logic(talent_score, genre_weight, season_multiplier, budget, target_market):
    ...
    base_score = (talent_score * 0.4) + (genre_weight * 0.3) + (target_market * 0.3)[cite: 4]

    # 2. Interaction Multiplier Block: Synergy between Veteran Talent and Festivals
    # Veterans + Festive releases (like Sankranthi) often over-perform[cite: 5]
    synergy_bonus = 1.0
    if talent_score > 90 and season_multiplier > 1.1:
        synergy_bonus = 1.15  # 15% boost for "Mega" synergy[cite: 5]

    # 3. Budget-Risk Penalty
    # High budgets without Pan-India appeal are mathematically riskier[cite: 4]
    risk_penalty = 1.0
    if budget > 100 and target_market < 80:
        risk_penalty = 0.85  # 15% penalty for budget-market mismatch[cite: 4]
